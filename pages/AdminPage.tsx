...
export const AdminPage: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!user) {
      alert("Please login with Strava first");
      return;
    }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: `Success! Updated ${data.count} users.`,
      });
    } catch (error) {
      setMessage({
        type: "error",
        text: "Failed to upload file. Please check the format.",
      });
    } finally {
      setUploading(false);
    }
  ...
  const downloadData = async () => {
    ...
    a.href = url;
    a.download = "leaderboard_data.xlsx";
    a.click();
  };
      ...
      <div className="space-y-8">
        ...
        <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
          ...
          <ol className="list-decimal list-inside text-sm text-blue-800 space-y-1">
            ...
            <li>
              Keep the "User ID" column unchanged to update existing users.
            </li>
        ...
        <div>
          ...
          {message && (
            <div
              className={`mt-4 p-3 rounded-lg text-sm ${message.type === "success" ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}
            >
              {message.text}
            </div>
          )}
